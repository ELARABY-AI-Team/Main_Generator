...
def upload_GPIO_to_github(mcu_name, h_content, c_content, folder_name):
    """Upload both GPIO.h and GPIO.c to GitHub in the specified folder"""
    if not GITHUB_TOKEN:
        return False

    h_file_path = f"{folder_name}/{mcu_name}_GPIO.h"
    c_file_path = f"{folder_name}/{mcu_name}_GPIO.c"
    ...
    h_data = {
        "message": f"Add {mcu_name}_GPIO.h",
        "content": h_encoded,
        "sha": sha_info[h_file_path]
    }

    c_data = {
        "message": f"Add {mcu_name}_GPIO.c",
        "content": c_encoded,
        "sha": sha_info[c_file_path]
    }
...
def check_github_for_GPIO(mcu_name, folder_name):
    """Check if GPIO files exist on GitHub in the given folder"""
    if not GITHUB_TOKEN:
        return None, None
...
class MainApp:
    ...
    def setup_ui(self):
        ...
        self.generate_config_btn.pack(side=tk.LEFT, padx=(10, 10))
        ...
        # Add this near your other buttons in setup_ui()
        self.generate_gpio_btn = tk.Button(
            left_buttons_frame,
            text="🔌 Generate GPIO.h/.c",
            command=self.on_generate_gpio_click,
            bg="#f39c12",
            fg="white",
            font=("Segoe UI", 10, "bold"),
            bd=0,
        ...
        self.generate_gpio_btn.pack(side=tk.LEFT, padx=(10, 10))
        ...
        # Tab for config.h preview
        self.config_h_tab = tk.Frame(self.notebook, bg="#34495e")
        self.notebook.add(self.config_h_tab, text="config.h")

        self.config_h_preview = ScrolledText(self.config_h_tab, bg="#1e272e", fg="#ecf0f1",
                                            insertbackground="white", font=("Consolas", 10),
                                            wrap=tk.NONE, padx=10, pady=10)
        self.config_h_preview.pack(fill=tk.BOTH, expand=True)

        # Tab for config.c preview
        self.config_c_tab = tk.Frame(self.notebook, bg="#34495e")
        self.notebook.add(self.config_c_tab, text="config.c")

        self.config_c_preview = ScrolledText(self.config_c_tab, bg="#1e272e", fg="#ecf0f1",
                                            insertbackground="white", font=("Consolas", 10),
                                            wrap=tk.NONE, padx=10, pady=10)
        self.config_c_preview.pack(fill=tk.BOTH, expand=True)

        # Tab for gpio.h preview
        self.gpio_h_tab = tk.Frame(self.notebook, bg="#34495e")
        self.notebook.add(self.gpio_h_tab, text="gpio.h")

        self.gpio_h_preview = ScrolledText(self.gpio_h_tab, bg="#1e272e", fg="#ecf0f1",
                                        insertbackground="white", font=("Consolas", 10),
                                        wrap=tk.NONE, padx=10, pady=10)
        self.gpio_h_preview.pack(fill=tk.BOTH, expand=True)

        # Tab for gpio.c preview
        self.gpio_c_tab = tk.Frame(self.notebook, bg="#34495e")
        self.notebook.add(self.gpio_c_tab, text="gpio.c")

        self.gpio_c_preview = ScrolledText(self.gpio_c_tab, bg="#1e272e", fg="#ecf0f1",
                                        insertbackground="white", font=("Consolas", 10),
                                        wrap=tk.NONE, padx=10, pady=10)
        self.gpio_c_preview.pack(fill=tk.BOTH, expand=True)
    ...
    def reset_system(self):
        ...
        # Reset GPIO button state
        self.generate_gpio_btn.config(state=tk.DISABLED, bg="#f39c12", fg="white")
        # Reset progress
        self.progress_bar["value"] = 0
        self.progress_label.config(text="Ready")
    ...
    def on_generate_click(self):
        ...
        def callback(result):
            ...
            if success:
                ...
                self.generate_gpio_btn.config(state=tk.NORMAL, bg="#1520A6", fg="white")  # Enable GPIO button
...
"""
    ...
    def on_generate_gpio_click(self):
        if not self.last_generated_mcu:
            return
            
        self.running = True
        self.generate_gpio_btn.config(state=tk.DISABLED)
        self.cancel_btn.config(state=tk.NORMAL)
        
        self.gpio_h_preview.delete(1.0, tk.END)
        self.gpio_c_preview.delete(1.0, tk.END)
        
        def callback(result):
            success, msg, h_content, c_content = result
            self.status_bar.config(text=msg)
            self.generate_gpio_btn.config(state=tk.NORMAL if success else tk.DISABLED)
            self.cancel_btn.config(state=tk.DISABLED)
            if success:
                self.display_code(h_content, "gpio_h")
                self.display_code(c_content, "gpio_c")
            else:
                messagebox.showerror("Error", msg)

        self.current_thread = threading.Thread(
            target=self.run_gpio_generation,
            args=(self.last_generated_mcu, callback),
            daemon=True
        )
        self.current_thread.start()
    ...
    def run_gpio_generation(self, mcu_name, callback):
        def progress(msg, val):
            self.progress_label.config(text=msg)
            self.progress_bar["value"] = val
            self.root.update_idletasks()
        ...
        try:
            # Get settings (adjust or remove if not needed for GPIO)
            current_date = datetime.datetime.now().strftime("%Y-%m-%d")
            voltage = self.settings.get('voltage', None)
            clock_source = self.settings.get('clock_source', None)

            gpio_folder = "GPIO_Files"

            # Check GitHub for existing GPIO files
            progress("Checking GitHub for existing GPIO files...", 10)
            h_url, c_url = check_github_for_GPIO(mcu_name, gpio_folder)

            if h_url and c_url:
                progress("Downloading GPIO files from GitHub...", 20)
                h_content = download_header_from_github(h_url)
                c_content = download_header_from_github(c_url)

                if h_content and c_content:
                    progress("Saving GPIO files...", 80)
                    base_path = filedialog.askdirectory(title="Select folder to save GPIO files")
                    if not base_path:
                        self.root.after(0, callback, (False, "Save canceled", None, None))
                        return

                    with open(os.path.join(base_path, f"{mcu_name}_GPIO.h"), "w", encoding="utf-8") as f:
                        f.write(h_content)
                    with open(os.path.join(base_path, f"{mcu_name}_GPIO.c"), "w", encoding="utf-8") as f:
                        f.write(c_content)

                    progress("Done! GPIO files loaded from GitHub.", 100)
                    self.root.after(0, callback, (True, f"GPIO files loaded from GitHub and saved to {base_path}", h_content, c_content))
                    return

            # Generate GPIO.h
            progress("Creating GPIO.h...", 30)
...
/* ==================== STRICT REQUIREMENTS ==================== */
1. SOURCING:
{f"Use the following PDF content to source port and pin definitions:\n{self.extracted_pdf_text}\n• Mark all values with /* PDF Reference */" if self.extracted_pdf_text else f"• Research {mcu_name} port/pin definitions from official datasheets or online sources\n• Mark all values with /* Assumed – please verify */"}
...
• Automotive/mission-critical safety level"""
            ...
            gpio_h_response = model.generate_content(gpio_h_prompt)
            gpio_h_content = self.clean_code_output(gpio_h_response.text)

            # Generate GPIO.c
            progress("Creating GPIO.c...", 60)
            gpio_c_prompt = f"""Create a clean, production-ready GPIO.c file for the {mcu_name} microcontroller.
    Implement all GPIO functions declared in GPIO.h using safe and standard practices.
    Include "{mcu_name}_GPIO.h" and "{mcu_name}_MAIN.h" at the top.
    Use proper initialization sequences, error handling, and comments.
    Add assumptions comments if necessary."""

            gpio_c_response = model.generate_content(gpio_c_prompt)
            gpio_c_content = self.clean_code_output(gpio_c_response.text)
            ...
            gpio_h_path = os.path.join(base_path, f"{mcu_name}_GPIO.h")
            gpio_c_path = os.path.join(base_path, f"{mcu_name}_GPIO.c")

            with open(gpio_h_path, "w", encoding="utf-8") as f:
                f.write(gpio_h_content)
            with open(gpio_c_path, "w", encoding="utf-8") as f:
                f.write(gpio_c_content)

            # Upload to GitHub
            if GITHUB_TOKEN:
                progress("Uploading to GitHub...", 90)
                upload_success = upload_GPIO_to_github(mcu_name, gpio_h_content, gpio_c_content, gpio_folder)
                if upload_success:
                    progress("✅ Done! Uploaded to GitHub.", 100)
                    messagebox.showinfo("Upload Successful", f"✅ {mcu_name}_GPIO.h/.c uploaded to GitHub.")
                else:
                    progress("⚠️ Done (upload failed)", 100)
                    messagebox.showwarning("Upload Failed", f"⚠️ Upload to GitHub failed for {mcu_name}_GPIO.h/.c.")
            else:
                progress("✅ Done!", 100)

            self.root.after(0, callback, (True, f"GPIO files saved to {base_path}", gpio_h_content, gpio_c_content))

        except Exception as e:
            self.root.after(0, callback, (False, f"GPIO generation failed: {str(e)}", None, None))
    ...
    def display_code(self, code, target):
        if target == "main":
            ...
            self.notebook.select(self.config_h_tab)
        elif target == "config_c":
            self.config_c_preview.delete(1.0, tk.END)
            self.config_c_preview.insert(tk.END, code)
            self.notebook.select(self.config_c_tab)
        elif target == "gpio_h":
            self.gpio_h_preview.delete(1.0, tk.END)
            self.gpio_h_preview.insert(tk.END, code)
            self.notebook.select(self.gpio_h_tab)
        elif target == "gpio_c":
            self.gpio_c_preview.delete(1.0, tk.END)
            self.gpio_c_preview.insert(tk.END, code)
            self.notebook.select(self.gpio_c_tab)
