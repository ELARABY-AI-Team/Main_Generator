...
#include "RENESAS_R5F11BBC_MCAL.h"      // Placeholder for device-specific header.
...
void Go_to_sleep_mode(void)
{
    ...
    __halt(); // Placeholder for assembly instruction to enter HALT mode.
...
{
    WDT_Reset(); // Clear WDT
    __EI(); // Enable Interrupts (Assembly instruction for many architectures)
...
{
    WDT_Reset(); // Clear WDT
    __DI(); // Disable Interrupts (Assembly instruction for many architectures)
 ...
 */
void I2C_Init(t_i2c_channel i2c_channel, t_i2c_clk_speed i2c_clk_speed, tword i2c_device_address, t_i2c_ack i2c_ack, t_i2c_datalength i2c_datalength)
{
    WDT_Reset(); // Clear WDT
