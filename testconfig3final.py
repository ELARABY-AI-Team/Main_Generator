...
class AdvancedSettingsDialog(tk.Toplevel):
    def __init__(self, parent, title, initial_values=None):
        ...
        # Settings container
        settings_container = tk.Frame(content_frame, bg='#ecf0f1', padx=20, pady=20)
        ...
        voltage_frame.pack(fill=tk.X, pady=(0, 15))
...
Output: A clean, production-ready `main.h` file."""
    ...
    def run_config_generation(self, mcu_name, callback):
        ...
        try:
            progress("Generating config files...", 10)
            ...
            # Generate config.h content with ALL settings
            progress("Creating config.h...", 30)
            config_h_prompt = f"""Create a config.h file for {mcu_name} microcontroller with:
- Voltage: {voltage}V
- Clock: {clock_source}
- Date: {current_date}

Include:
1. Header guard matching {mcu_name}_CONFIG_H_
2. All hardware-specific configuration defines
3. Peripheral initialization macros
4. Clock configuration for {clock_source}
5. Pin mapping definitions
6. Error code definitions
7. Voltage-specific settings for {voltage}V operation
8. Hardware abstraction layer declarations
9. Function prototypes for all configuration functions
10. Documentation for each configuration option"""

            response = model.generate_content(config_h_prompt)
            config_h_content = self.clean_code_output(response.text)
            
            # Generate config.c content with ALL settings
            progress("Creating config.c...", 60)
            config_c_prompt = f"""Create a config.c file for {mcu_name} microcontroller that complements the config.h.

Include:
1. Implementation of all functions declared in config.h
2. Peripheral initialization code for {voltage}V operation
3. Clock setup for {clock_source}
4. Hardware abstraction layer implementation
5. Error handling implementation
6. Voltage-specific implementations
7. Detailed comments explaining each configuration step
8. Register initialization sequences
9. Safety checks for {voltage}V operation
10. Clock source validation for {clock_source}"""
